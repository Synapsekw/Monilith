-- Phase 5a fix: current_setting(name, true) returns '' (not NULL) for a custom GUC
-- once registered on a pooled connection; ''::int raised 22P02 and aborted every
-- cell_values write on boards with an automation. Guard with nullif(...,'').

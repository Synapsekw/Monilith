-- Status intelligence (descoped): percent_reached trigger + set_percent action.
--
-- CREATE OR REPLACE only — no schema/type change. Two engine functions are
-- recreated with their bodies copied verbatim from their latest definitions,
-- each gaining exactly one branch:
--   * tg_run_automations (from 20260619100000_automations_5c1_run_history.sql)
--     gains the `percent_reached` trigger match — fires only on
--     threshold-CROSSING percent writes (INSERT, or old below the threshold),
--     so a non-crossing rewrite (e.g. 100 -> 100) never fires.
--   * _automation_run (from 20260622130000_automation_move_to_group.sql)
--     gains the `set_percent` action — writes {"percent": n} to the target
--     cell, idempotent: an equal current value logs `skipped_equal` instead
--     of writing.
--
-- Loop analysis (Completed <-> 100% two-way sync, both recipes enabled):
--   status -> Done fires status_changed -> set_percent 100 writes the percent
--   cell, which re-enters tg_run_automations at depth+1 -> percent_reached
--   fires (crossing) -> set_option Done finds the status cell already equal
--   -> `skipped_equal`, no write, chain ends at depth 2. The reverse
--   direction terminates symmetrically. `pulse.aut_depth` (bail at >= 5)
--   remains the backstop.

-- 1) cell_values trigger: + percent_reached (crossing semantics).
--    Body copied verbatim from 20260619100000_automations_5c1_run_history.sql
--    with one added disjunct in the rule-matching where clause.
...
-- 2) Shared action runner: + set_percent (idempotent).
--    Body copied verbatim from 20260622130000_automation_move_to_group.sql
--    with one added branch after set_option.

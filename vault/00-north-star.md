...
- **Owed — three items, all needing _you_ rather than an agent.** **`digest_secret` is still absent from prod Vault**, but the schema half is now live, so provisioning is finally safe: ping `_health_digest_ping()`, confirm a `blocked` row naming the secret appears (the alarm that was missing for three weeks), then add it. Note prod has **no `RESEND_API_KEY`**, so the first runs file in-app notifications and send **no email** regardless — inspect `digest_runs.stats` before adding one. **E5 semantic search is still inert** (`item_embeddings` = 0 vs 439 live prod items) — but `OPENAI_EMBEDDING_API_KEY` **was** provisioned on Vercel 2026-07-25; the old "denied by the permission classifier" note was stale. All that remains is enqueueing existing items into `item_embed_queue`, after which the `embed-sweep` cron drains 50 every 2 min and signs its own call — no secret handling needed. **`supabase-prod` MCP is read-only (`25006`)**, so that enqueue must be pasted into the prod SQL editor. **The Ask Pulse write path is still unexercised on prod.**

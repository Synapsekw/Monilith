...
- **In flight:** nothing open in this session. Last edit: **Timeline two-column spans + colorize** ([[2026-06-24-2232-timeline-spans-colorize]]) — Gantt draws spans between a chosen start/end date column (one date → dot, none → unscheduled) and colors bars/dots by a status/dropdown column; new view-config keys, no migration, 0-round-trip pickers; merged unit-gated (integration gate bypassed — flaky). Other sessions' worktrees are live (`calendar-redesign`, `streaming-shell-9-2`); their unrelated uncommitted edits sit in the main checkout.

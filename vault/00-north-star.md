...
- **Phase:** **MCP server (agent connectivity) shipped to `develop`** ([[2026-07-24-1950-mcp-server-oauth]]). Hosted remote MCP server + OAuth 2.1 authorization server so Claude Desktop/claude.ai can connect as a real Pulse user — genuinely RLS-scoped per-request session (no service-role bypass), 6 read/write tools, no delete. Built subagent-driven (14 tasks, per-task review loop + final whole-branch review); the final review caught a real concurrency bug (session refresh racing under concurrent tool calls, could brick a connection) and two other Important gaps (Vault secret not freed on revoke, missing RFC 9728 discovery route) — all fixed and re-verified before merge (`86f34fd`). The Timeline zoom levels + sub-item nesting + timestamp date sources + Vitest-shebang work ([[2026-07-17-2044-timeline-upgrades-hook-test-fix]]) already promoted to prod since — **`main` was found to be at `8b4d648` (PR #69), not the `4235d96` this doc previously tracked**, i.e. one promotion cycle happened without a north-star bump; corrected here.
- **Branch:** `develop` @ `86f34fd` == `origin/develop` (MCP server + E5 agentic/semantic ahead of prod — everything else already promoted via #69). **`main` @ `8b4d648`**. All `task/*` worktrees merged + cleaned.
- **In flight:** none building.
- **Next:** a real end-to-end connection test (Claude Desktop/claude.ai against a deployed `develop`) hasn't been run yet for the MCP server — do that before calling it done-done. Otherwise: **promote `develop → main`** (deferred to the owner) to carry MCP server + E5 to prod (the only two features still ahead) — E5 needs its env vars set + an embeddings backfill first (see Owed). Or a fresh roadmap build — **Report Builder v2** (charts + wide-board table) or **E6** (Stripe billing; F16 blocked on your creds).
- **Owed:** **MCP server follow-ups:** `/login` doesn't honor `?next=` so the OAuth connect flow can't resume post-login for a not-yet-signed-in user (pre-existing app-wide gap, first flow it actually bites); revoke-button errors aren't surfaced in Settings UI; `/api/oauth/register` has no rate limit. **E5 pre-promotion:** set E5 env vars, then backfill embeddings (`POST /api/ai/embed?mode=backfill`) so semantic surfaces actually return results once promoted — the build-session's own session note was never committed to this vault (written on a different checkout), so this line is the only record of what's left. **Report Builder v2** = charts + wide-board table continuation. **Wordmark mark** in prod — one-line revert in `brand.tsx` if busy. **Rotate the prod DB password** (still owed from an earlier `/sync-prod`).

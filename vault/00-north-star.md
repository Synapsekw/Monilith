...
- **9 — Hardening & Optimization** — <span style="color:#eab308">**[In progress — 9.1 + 9.2 + 9.3 + 9.4 done; 9.3b + 9.6 next]**</span> — Perf + perceived-perf program (Track A actual speed, Track B perceived speed, closing Web-Vitals gate 9.6). 9.1 auth fast-path + 9.2 streaming shell + **9.3 tagged `use cache` shell reads** + **9.4 route skeletons** shipped; 9.3b dashboard-widget aggregation caching (deferred) + 9.6 Web-Vitals gate next. Spec `docs/superpowers/specs/2026-06-22-phase-9-performance-optimization-design.md`.
...
- **Phase:** Phase 6 (ClickUp depth) complete except deferred 6e Docs · Phase 7 (Asana polish) in progress — 7a + 7b + 7c + Workload analytics v3 + **time allocation (`/time` My Time card + full-canvas Workload)** shipped · Phase 9 (Hardening) in progress — 9.1 + 9.2 + **9.3 cache + 9.4 skeletons** shipped, 9.3b + 9.6 next.
- **Branch:** `develop == origin/develop` at `6d90701` (squash-heal merge on top of the promotion). Last production promotion is **#35** (2026-06-25, `674c570` on `main` — calendar item-panel open, calendar canvas-fill, Phase 9.4 route skeletons, Phase 9.3 shell read caching). `main ⊆ develop` (healed, gotcha-32). `develop` is even with `main` in content. ([[2026-06-25-2052-batch-a-build-and-promote]])
- **In flight:** nothing open. Last shipped: Batch A (onOpenItem `972aea7`, Phase 9.4 skeletons `b855bb8`, Phase 9.3 cache `5fa2d16`) built in parallel worktrees + promoted as #35. Two other-session worktrees still live (`streaming-shell-9-2` stale, `board-spreadsheet-io`); the unrelated uncommitted edits (`DashboardsNav`, deleted `GenerateWithAiButton`) sit in the main checkout, untouched.
- **Next:** 9.3b dashboard-widget aggregation caching (deferred; React-Query-wrapped Server Actions) → 9.6 Web-Vitals gate → optional carryover: make board rename/delete/create/duplicate optimistic (shared client store; must rebase onto the now-landed `actions.ts` `updateTag` lines).
- **Owed:** Re-run the integration suite for the unit-gated merges once Supabase is stable. **Integration-test gate is flaky AND writes to production** — the live-DB `*.integration.test.ts` suites run against the **remote/prod** Supabase (Vitest loads `.env.local`), so they fail nondeterministically _and_ leak fixtures (`*@example.com` users, `Org a`/`rls-*`/`eng5b1-*` orgs) into real tenant data when a test throws before teardown. Timeout-under-load is now partly mitigated (`testTimeout/hookTimeout/retry` on the integration project, `1e93f03`), but the real fix is unchanged: local Docker Supabase (`supabase start` → `127.0.0.1:54321`) via `.env.test` — isolates + makes the gate reliable ([[2026-06-23-gotcha-43-shared-db-integration-test-flake]], [[2026-06-24-0751-test-fixture-prod-db-cleanup]], [[2026-06-24-2311-integration-test-timeout-flake]]).

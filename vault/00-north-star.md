...
- **MVP-F — MVP Final Features (user-feedback backlog)** — <span style="color:#eab308">**[In progress — Batch A 6/6 shipped; Batch B pending]**</span> — All open in-app feature requests consolidated into `docs/superpowers/plans/2026-07-03-mvp-final-features.md` (`/goal` points here). Batch A merged: formatted Excel export, calendar/timeline quick-edit peek, overdue tint + percent sync, currency column + UAE dirham sign, column drag-reorder, completion dashboard widget. Batch B open: summary row, priority auto-critical, health summary + weekly digest. ([[2026-07-03-1154-mvp-final-features-wave]])
...
- **Phase:** **MVP-F Batch A DONE on develop** — all six user-feedback features merged in one 6-wide parallel worktree wave ([[2026-07-03-1154-mvp-final-features-wave]]) · Phase 7 (Asana polish) in progress · Phase 9 + TOUCH + perf passes done · current product goal = `docs/superpowers/plans/2026-07-03-mvp-final-features.md` (`/goal`).
- **Branch:** `develop == origin/develop` at `0abb4d3` (6 MVP-F merges: `7ae8e7a` excel-export, `2fa1138` calendar-peek, `96c0f32` overdue+percent-sync, `bd9217b` currency, `b58f5f5` column-reorder, `0abb4d3` completion-widget) — **promotion to main in progress this session** (main behind at #45).
- **In flight:** promote develop → main; then flip feedback rows F1/F3/F5/F6 to `resolved`.
- **Next:** (1) finish the promotion + confirm prod deploy; (2) dispatch **MVP-F Batch B** (summary row ∥ priority auto-critical ∥ health summary + digest) per the goal plan's DAG.
- **Owed:** prod DB needs the five dev-applied migrations (`20260702120000` perf + four MVP-F) via `/sync-prod` + `migration repair` (applied through SQL editor, ledger unrecorded — [[2026-07-03-gotcha-43-parallel-branch-migration-version-collision]]). Four dead lock-held worktree dirs under `.claude/worktrees/` to delete. Untracked `scripts/sync-prod/push-schema.sh` of unknown origin — investigate. `.mcp.json` local edit still uncommitted. Perf tier-3 leftovers per [[2026-07-02-1902-perf-pass-four-parallel-worktrees]].

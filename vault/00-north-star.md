...
- **Branch:** `develop == origin/develop` at `afe5019` (vault commits may sit on top locally). Last production promotion is **#30** (2026-06-23, `f10cd8c` on `main` — Workload v2 filtering+actuals, time-tracking date Calendar, people-cell names). `develop` is **ahead of `main`** since #30 (Phase 9.2 streaming shell, Workload analytics v3, Kanban card names, auth `getClaims` proxy, sidebar board drag-reorder, Feedback reporting, Dashboards v2 polish, percent (progress) column **+ value-based color + collapsed-group rollup**, time allocation, column-add error surface, Sum/Average measure fix, `/updates` changelog backfill, shared `(app)` shell layout, reorder no-shell-reload, **AI dashboard generation (Opus 4.8)**, Kanban card polish (Direction A) — none promoted yet).
- **In flight:** nothing open in this session. Last edit: **percent-column colorization + collapsed-group rollup** ([[2026-06-24-2112-percent-color-and-group-rollup]]) — red→green OKLCH bands on the percent bar (leaf + parent + group rollup) and an "Average" summary row on group collapse; both merged unit-gated (integration gate bypassed — flaky). Other sessions' worktrees are live (`calendar-redesign`, `timeline-spans`, `streaming-shell-9-2`); their unrelated uncommitted edits sit in the main checkout.

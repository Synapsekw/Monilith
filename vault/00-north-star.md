...
- **Next:** **promote `develop → main`** (`/promote`) to ship the audit-fix sweep + summary footer + nav declutter to prod. Then the big new thrust is **Phase 10 — AI & Agents, Epic 1** (foundation + Ask Pulse) — scoped + specced this session ([[2026-07-05-decision-26-ai-platform-dual-billing]]); its implementation plan is the immediate next artifact. Migration-gated deferrals still queued behind it (write schema → user applies → build slice): soft-delete/undo (`archived_at`), avatar upload (`avatars` bucket), similarity-ranked ⌘K search.

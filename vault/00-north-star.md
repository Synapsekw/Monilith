...
- **10 — AI & Agents** — <span style="color:#eab308">**[Scoped — E1 spec written; plan next]**</span> — Reusable AI platform layer (gateway resolving **managed vs bring-your-own-key** · Supabase-Vault BYO store · `ai_usage` ledger + credits · entitlements) + a feature wave (Ask Pulse workspace Q&A, item assist, generation, agentic automations, semantic search). Sold two ways: included-in-plan **or** BYO key. Scope `docs/superpowers/specs/2026-07-05-ai-platform-phase-10-scope.md`; E1 spec `…-ai-foundation-and-ask-pulse-design.md`. 6 epics; E1 (foundation + Ask Pulse) is the ~2-week critical-path slice. ([[2026-07-05-decision-26-ai-platform-dual-billing]])
...
- **Branch:** `develop == origin/develop` at `66cc2f1` (shadcn charts P1+P2 + keystone brand + cross-group-dnd merged on top of nav-declutter/summary-footer/audit-fix) · `main` at `f0f71f5` — **develop is ahead of main; promotion pending.** DEV migration ledger Local==Remote, zero drift. **Import Wizard Structure step merged to develop/origin.** Several other sessions active in their own worktrees (perf-tier3, pwa-shell, rename-board-shared-tag, widget-preview-live).
- **In flight:** **Nav declutter (Direction B) shipped** — grouped, collapsible sidebar with a top workspace switcher that scopes Boards+Dashboards to an active workspace, platform admin consolidated into one top-right button, dead Inbox removed, workspace rename/delete moved to Settings ([[2026-07-05-1456-nav-declutter-direction-b]], `finish-task-merge-blocked-by-concurrent-uncommitted`). Summary-footer + boards UI polish landed earlier this day ([[2026-07-05-1218-percent-summary-fill-bar]], [[2026-07-05-1227-boards-summary-ui-polish]]). **Import Wizard Structure step shipped** this block — 4-step wizard (Upload→Map→**Structure**→Confirm) with per-row item/subitem + multi-group assignment + bulk actions, wider modal, pinned action footer, and a multi-group `import_rows_into_board` migration (applied to DEV) ([[2026-07-05-1458-import-wizard-structure-step]]). **Cross-group item drag-and-drop (Table view) shipped** — unified board-level `DndContext` lets items drag between groups at the exact drop spot (empty/collapsed groups append); reuses `moveItem`/`moveItemToGroup` extended with an optional position ([[2026-07-05-1903-cross-group-item-dnd]]). Needs a real-browser pass (jsdom can't test collision selection) before promote. **Board date hydration mismatch fixed** — `toLocaleDateString(undefined, …)` in three SSR'd board cells (footer/rollup/leaf) rendered `Jan 1` server vs `1 Jan` browser; pinned `"en-US"` ([[2026-07-05-1904-fix-tolocaledatestring-hydration]], [[2026-07-05-gotcha-50-tolocaledatestring-undefined-locale-hydration-mismatch]]). **Brand refresh (Keystone) shipped** — the Nunito wordmark with the I recut as the monolith slab + a distinct cleaved-monolith standalone mark, applied to the app (`Brand`, `MonolithMark`, `icon.svg`, `apple-icon`, regenerated PWA/email rasters via new `scripts/generate-app-icons.ts`) and merged to develop; an untracked-but-committed `brand-lab/` holds the exploration incl. four landing variations awaiting a pick ([[2026-07-05-2005-brand-keystone-logo-favicon]]).
- **Next:** **promote `develop → main`** (`/promote`) to ship the audit-fix sweep + summary footer + nav declutter to prod. Then the big new thrust is **Phase 10 — AI & Agents, Epic 1** (foundation + Ask Pulse) — scoped, specced, **and planned** this session ([[2026-07-05-1746-phase-10-ai-roadmap-scope]], [[2026-07-05-decision-26-ai-platform-dual-billing]]); spec + TDD plan are ready to dispatch via `/whats-next` into a `task/ai-foundation-ask-pulse` worktree (Task 0 migration is user-applied). Migration-gated deferrals still queued behind it (write schema → user applies → build slice): soft-delete/undo (`archived_at`), avatar upload (`avatars` bucket), similarity-ranked ⌘K search.
- **Owed:** three migration-gated feature deferrals above. Four nav-declutter follow-up Minors (redundant admin `countNewFeedback` query in the sidebar loader; dead `WorkspaceSwitcher.isOrgAdmin`; `NavSection` a11y polish; dashboards workspace-filter test — see [[2026-07-05-1456-nav-declutter-direction-b]]). `.mcp.json` uncommitted; untracked `scripts/sync-prod/push-schema.sh` (foreign). Perf tier-3 **Task A** (`unstable_instant`) deferred — needs its own spec ([[2026-07-04-gotcha-48-unstable-instant-blocked-by-shell-searchparams]]). Prod Batch B feedback rows F2/F5 flip after next prod confirm. Import follow-ups (non-blocking): drop dead `buildImportPayloadV2`/`splitRows2`; export→import `↳`-marker round-trip asymmetry (intended); watch DEV migration-ledger drift on `20260705120000` (applied manually). **Landing redesign: pick one of four `brand-lab` landing variations (Statement/Product/Editorial/Kinetic or a hybrid) then apply it to `/landing`** — brand-lab is committed for portability and should be removed once the landing lands. **Charts P2 follow-up (perf, non-blocking):** `WidgetConfigSheet` statically imports `ChartWidget`, pulling recharts + the new chart modules into the dashboard's first-paint chunk despite `DashboardWidget`'s `dynamic()`; the guard test only greps its own source (false confidence). Dynamic-import the preview + extend the guard ([[2026-07-05-2018-shadcn-charts-phase1-2-expressive]]).

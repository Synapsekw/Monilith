...
- **5 — Automations + Rules** — <span style="color:#22c55e">**[Done — 5a + 5b-1 + 5b-2 + 5c-1 + 5c-2]**</span>
  ...
  _**5c-2** (Done 2026-06-19, subagent-driven, **closes Phase 5**): external **webhook actions** — a
  `call_webhook` action enqueues an HTTPS POST via **`pg_net`** (`queued`) + an
  `automation_webhook_deliveries` ledger; a 1-min `pg_cron` **`_automation_webhook_reconcile`** folds
  `net._http_response` into the 5c-1 run-history (`delivered_<code>`/`failed*<code>`/`failed_network`).
Baseline SSRF guard (`\_webhook_url_safe`) + an **admin-gate DB trigger** (42501, `auth.uid()`-guarded
  so trusted contexts pass). 642 tests + 7-case cloud integration + e2e + a real-network reconcile
  proof (405/503/200 → mapped); final whole-branch review **Ship**. Spec
  [[2026-06-19-phase-5c2-automations-design]]; see [[2026-06-19-1316-phase5c2-webhook-actions]].*
  ...
  **date-based** triggers via `pg_cron` (which turned out to be available — 1.6.4) — a `date_reached`
  ...
  - RLS) + e2e; advisors clean; holistic review SHIP-WITH-NITS (emerald→`bg-primary` badge + blocked/error
    ...
    [[2026-06-19-0957-phase5c1-run-history]]. **5c-2** next: external/webhook actions via `pg_net`.\_

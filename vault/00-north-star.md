...
- **In flight:** **`/sync-prod` mid-run** — schema parity + independent-prod-data guard passed (PROD 10 orgs/10 users ⊆ DEV; no prod-native data); user confirmed `SYNC PROD`; the 4 handover commands (backup → dump → restore → storage) were handed over and are running/awaited. NOTE: DEV is padded with ~220 test-fixture orgs/users that a full replace pushes to prod.
- **Next:** finish the sync-prod parity verification, then **finish TOUCH Batch 2**: the two remaining surfaces still on old `PointerSensor` with `TODO(touch-batch-2)` markers — **`GanttBoard.tsx`** (bar drag) + **`CalendarBoard.tsx`** (event drag); `ColumnOptionsDialog`/`BoardsNav` markers were already cleared by the Table/Nav passes. Then Phase 7 remaining polish / Phase 10. Optional carryover: optimistic board mutations — **recommended skip**; `/roast` follow-ups — minor.

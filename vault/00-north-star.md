...
- **Branch:** `develop == origin/develop` (`77828be`, pushed 2026-06-21 — migration-ledger drift fix + the 3 rebased phase-7a docs commits, on top of the worktree-per-session workflow; gate green). `main` at `30a9cf3` (Phases 0–4) — **do not promote yet** (WebGL landing dep needs a manual cross-browser check). _(Parallel sessions share this checkout — a concurrent branding edit to `page.tsx` is uncommitted in the tree; leave it untouched; commit only your own paths.)_

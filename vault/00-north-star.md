...
- **Branch:** local `develop` is **11 commits ahead of `origin/develop` (`531badf`) and NOT pushed** — 7 are 6f PDF-preview (mine, green in isolation), interleaved with ~5 from a concurrent board-sharing session whose `listBoards→listMyBoards` rename is half-wired (HEAD has ~12 typecheck/build errors, all in board-sharing files: `page.tsx`, `sidebar.tsx`, layouts, command-palette/app-shell tests). **Push waits until that session compiles.** `main` at `30a9cf3` (Phases 0–4) — **do not promote yet** (WebGL landing dep needs a manual cross-browser check). _(Parallel sessions share this checkout — leave their edits untouched; commit only your own paths.)_

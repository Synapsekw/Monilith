...
- **Phase:** Phases 0–4 complete + promoted to `main`. **Phase 8 COMPLETE**. **RS workstream COMPLETE** (dark + light). **Phase 5 (Automations + Rules) COMPLETE — 5a + 5b-1 + 5b-2 + 5c-1 + 5c-2 (webhook actions) all shipped 2026-06-19.** Side: **board group management COMPLETE 2026-06-19**. **Phase 6 (ClickUp depth) STARTED — 6a Subitems shipped 2026-06-19** (single-level nesting reusing `items.parent_id` + a DB single-level trigger; subitems share the board's columns; Table nesting + collapsed rollups + drag-reorder). **Next: 6b — custom fields/statuses** (then C time-tracking, D relations+mirror, E docs).

...
- **Phase:** Phases 0–4 complete + promoted to `main`. **Phase 8 COMPLETE**. **RS workstream COMPLETE** (dark + light). **Phase 5 (Automations + Rules) COMPLETE — 5a + 5b-1 + 5b-2 + 5c-1 + 5c-2 (webhook actions) all shipped 2026-06-19.** Side: **Name-column auto-fit + manual resize** + **Add-group (board groups creatable from the Table)** both shipped + user-verified 2026-06-19. **Next:** **Phase 6 (ClickUp depth: subitems/nesting, time tracking, Docs, custom fields, relations+mirror)** — large phase, slice via brainstorming.
- **Branch:** `develop` **pushed to `origin/develop` 2026-06-19** (`…991bbea`; 5c-1 run-history + name-column + add-group + **5c-2 webhook actions**, interleaved with a parallel session's dashboards/group-reorder commits — all now on origin). `main` at squash commit `30a9cf3` — **do not promote yet** (WebGL landing dep needs a manual cross-browser check).

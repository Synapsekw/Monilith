...
- **Phase:** **All four gates were green, which is exactly when debt stops being visible — so the audit went after what gates structurally cannot see** ([[2026-08-01-1146-debt-audit-and-paydown]]). Two more reachable-but-uncalled `"use server"` endpoints (`bulkDeleteItems`, `bulkPurgeItems` — one a bulk **hard delete**), eight untested server-action modules, and a migration-ledger gate that had not run for two sessions. Both branches shipped: **+107 net tests** (3669 → **3776**), two endpoints closed, two real bugs fixed.
...
- **Branch:** `main` @ `1121796` == production. `develop` @ `59997249` == `origin/develop` — the changelog trailers, the 2026-07-31 sweep, and today's two merges (`57e2c4a4` debt paydown, `59997249` contract fixes). Promotion is **documentation plus two dead endpoints removed plus new test coverage** — behaviour-neutral **except one error path**: a DB/RLS failure in the status-column picker now returns `{ok:false}` to the picker's own error state instead of rejecting into Next's error boundary. Ledger **verified this session: 118 files = 118 DEV rows, no drift** (PROD unverified — needs the prod ledger read). All `task/*` worktrees merged + cleaned; the `landing-agentic` worktree belongs to a concurrent session.
- **In flight:** none.
- **Next:** **run Phase A of `docs/superpowers/plans/2026-08-01-debt-paydown-and-promotion.md`** (owner-executed: verify the two Vault secrets, enqueue, confirm the queue actually shrinks), **then promote `develop → main`** — two changelog entries announce semantic search, which returns nothing on prod until the queue drains. Then **Report Builder v2 roll-ups + org templates** as the critical path — one migration dropping `reports.board_id NOT NULL`, after which the single-board assumption must be unwound across access checks, payload fetch, shaping, routing and ~1.4k lines of tests. **E6** Stripe remains blocked on your creds.
- **Owed — three items, all needing _you_ rather than an agent.** **E5 semantic search is still inert** (`item_embeddings` = 0 vs 439 live prod items) and is a **promotion blocker**: the changelog announces "Find similar items" and "Ask AI searches by meaning" the moment `main` advances. **Correction to the previous revision of this bullet — "no secret handling needed" was wrong.** `_embed_sweep_ping()` reads `app_url` **and** `ai_pgnet_hmac_secret` from Vault; if either is absent it calls `raise notice` and returns, and **a notice is not an error — `cron.job_run_details` records the job as succeeded**. Enqueue 439 rows against an under-provisioned Vault and every sweep reports green forever while the queue never drains. A second silent edge: the endpoint verifies with `AI_PGNET_HMAC_SECRET` on Vercel, which must _equal_ the Vault copy; on mismatch it 401s but `net.http_post` is fire-and-forget, so the DB never learns. **Verify preconditions first, enqueue second, confirm the queue is actually shrinking third** — the gated runbook is Phase A of `docs/superpowers/plans/2026-08-01-debt-paydown-and-promotion.md`. **`supabase-prod` MCP is read-only (`25006`)**, so it is pasted into the prod SQL editor. **`digest_secret` is still absent from prod Vault**, but the schema half is live, so provisioning is finally safe: ping `_health_digest_ping()`, confirm a `blocked` row naming the secret appears, then add it — noting prod has **no `RESEND_API_KEY`**, so the first runs file in-app notifications and send **no email** regardless. **The Ask Monolith write path is still unexercised on prod.**
- **Left standing on purpose:** the `max-lines` tripwire on `landing-sections.tsx` (837/800) — it keeps god-file accretion visible until a split is scheduled, so silencing it would defeat it. Deliberately untouched: **309** symbols exported but referenced only inside their own module (169 types / ~140 runtime — mostly _over-export_, not dead code: `resolveAiAdapter`, `getBoardHandler`, `HomeDispatch` are all used in-file), and the `as unknown as Json` casts on `.from().insert()/.update()`, which `typedRpc` structurally cannot wrap. **Now pinned by tests rather than fixed:** `archiveBoard`'s non-owner message says "delete" (the UI surfaces archiving as Delete, so it may be deliberate); `reorderColumn` / `resizeNameColumn` delegate org-scoping **entirely to RLS** with no application-layer check; `getBoardStatusColumns` drops a column's **whole** option list when one option fails validation; `deleteBoard` / `purgeBoard` free Storage _after_ the row delete, orphaning objects if the cleanup throws. Also 29 dependencies behind, all patch/minor, no majors.

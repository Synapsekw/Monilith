...
- **Phase:** **Batch-A cleanups done — 3 parallel-worktree tasks merged to develop.** `/whats-next` scoped-then-built three disjoint carryover follow-ups ([[2026-07-06-1006-whats-next-batch-a-cleanups]]): charts-P2 first-paint lazy-load (`LazyChartWidget` + transitive-import guard test), nav-declutter minors (redundant `countNewFeedback` round-trip dropped, dead `isOrgAdmin` removed, `NavSection` a11y, dashboards workspace-filter test), and import dead-code removal (`buildImportPayloadV2`/`splitRows2`). Prior thrust: audit-fix sweep — closed intra-org RLS leak + SSRF/redirect/enumeration, killed hot-path revalidation + presence storm, +6 features — **already promoted to prod as PR#51** ([[2026-07-05-0811-audit-fix-sweep]]).
- **Branch:** `develop == origin/develop` at `e6afab1` · `origin/main` at PR#51 `e17973c` (audit-fix sweep) — **develop ahead of main; promotion pending** (charts P1+P2, keystone brand, cross-group-dnd, hydration fix, + Batch-A cleanups all stacked). DEV migration ledger Local==Remote, zero drift. **No live worktrees / `task/*` branches** — tree is clean.
- **In flight:** nothing building — all worktrees closed. **Promotion is the open loop:** the only pre-promote gate is a **cross-group-DnD real-browser pass** (jsdom can't test collision selection — [[2026-07-05-1903-cross-group-item-dnd]]). Everything else on the promote stack already merged + gated: charts P1+P2 ([[2026-07-05-2018-shadcn-charts-phase1-2-expressive]]), keystone brand ([[2026-07-05-2005-brand-keystone-logo-favicon]]), board date hydration fix ([[2026-07-05-1904-fix-tolocaledatestring-hydration]]), nav declutter Direction B ([[2026-07-05-1456-nav-declutter-direction-b]]), import wizard Structure step ([[2026-07-05-1458-import-wizard-structure-step]]), summary footer ([[2026-07-05-1218-percent-summary-fill-bar]]).
- **Next:** cross-group-DnD browser pass → **promote `develop → main`** (`/promote`). Then the big new thrust is **Phase 10 — AI & Agents, Epic 1** (foundation + Ask Pulse) — scoped, specced, **and planned** ([[2026-07-05-1746-phase-10-ai-roadmap-scope]], [[2026-07-05-decision-26-ai-platform-dual-billing]]); spec + TDD plan ready to build (`/develop`) in a `task/ai-foundation-ask-pulse` worktree (Task 0 migration user-applied). **Landing redesign** also ready once a `brand-lab` variation is picked. Migration-gated deferrals queued behind Phase 10 (write schema → user applies → build slice): soft-delete/undo (`archived_at`), avatar upload (`avatars` bucket), similarity-ranked ⌘K search.
- **Owed:** three migration-gated feature deferrals above. Unsuffixed `buildImportPayload` (0 prod call sites, separate lineage — retire or confirm intentional keep). Perf tier-3 **Task A** (`unstable_instant`) deferred — needs its own spec ([[2026-07-04-gotcha-48-unstable-instant-blocked-by-shell-searchparams]]). Prod Batch B feedback rows F2/F5 flip after next prod confirm. Import follow-up (non-blocking): export→import `↳`-marker round-trip asymmetry (intended). **Landing redesign: pick one of four `brand-lab` landing variations (Statement/Product/Editorial/Kinetic or a hybrid) then apply it to `/landing`** — brand-lab is committed for portability, remove once the landing lands. Two stale `_draft-*.md` (07-05 1038, 1606) left for their own blocks' wrapups.

...
- **Branch:** `develop == origin/develop` at `4dc3016`; `main ⊆ develop`. **Unpromoted on develop since #38:** TOUCH Batch 2 four-surface parallel batch — Kanban (`ba2b263`), Item Panel (`2274252`), Nav+a11y (`b60d4af`), Table (`4dc3016`) + changelog regen (`45a2bbe`). CI green at `4dc3016`. **Last production promotion is #38** (2026-06-28, `3423ee9`): iPad touch foundation (Batch 1) + 9.6 Web-Vitals gate + 9.3b widget-aggregation cache + `/roast` decision-council + spreadsheet review follow-ups. (Preceded by **#37**, `19e86e0`.) After #38, CI was tuned: Lighthouse budget made **informational/warn-level** (`94aea1c`, `ba4981e`) and CI verify runs **unit-only** (`4da5c30`).
- **In flight:** nothing open; main-checkout tree clean (only unrelated `.obsidian/*` edits, left untouched).
- **Next:** run `/promote` to ship the unpromoted TOUCH Batch 2 bundle, then **finish TOUCH Batch 2**: the two remaining surfaces still on old `PointerSensor` with `TODO(touch-batch-2)` markers — **`GanttBoard.tsx`** (bar drag) + **`CalendarBoard.tsx`** (event drag); `ColumnOptionsDialog`/`BoardsNav` markers were already cleared by the Table/Nav passes. Then Phase 7 remaining polish / Phase 10. Optional carryover: optimistic board mutations — **recommended skip**; `/roast` follow-ups — minor.
- **Owed:** **Migration-ledger repair** — `20260625120000_item_created_by.sql` is now committed **locally** but was applied to the DEV DB via the SQL editor (agent can't run DDL), so the remote ledger doesn't record it as applied; a future `supabase db push` will try to re-apply it. **You** run `supabase migration repair --status applied 20260625120000` when reconciling the pre-existing ledger drift (the remote already has 3 versions absent locally). See [[2026-06-26-1044-item-creation-metadata]]. _Integration-test gate:_ cross-run teardown cascade flake is FIXED (`56180fb`; [[2026-06-26-1014-integration-flake-root-cause]]); the live-DB suites still run against remote Supabase — durable fix is a dedicated isolated test database (a separate test-only Supabase project) via `.env.test` ([[2026-06-23-gotcha-43-shared-db-integration-test-flake]]). _finish-task vs CI changelog gap:_ `finish-task.sh` gates on `typecheck/lint/test/build` but **not** `pnpm changelog:gen`, while CI has a `develop`-only **changelog-drift** job that fails if `src/lib/changelog/generated.ts` is stale vs the `Changelog:` commit trailers. TOUCH Batch 2 (Kanban) tripped this — local gate green, CI red — fixed by a manual regen commit (`45a2bbe`). **Fix forward:** add `pnpm changelog:gen` (+ stage generated.ts) to `finish-task.sh`, or have build agents run it before finishing whenever a commit carries a `Changelog:` trailer.

...
- **6 — ClickUp depth** — <span style="color:#eab308">**[In progress — 6a subitems done]**</span>
  ...
  **6a — Subitems** (Done 2026-06-19): single-level nesting reusing `items.parent_id` + a `tg_items_single_level` BEFORE-trigger; subitems share the board's columns; Table nesting (dnd-sortable sub-block + read-only rollups on collapsed parents); new `addSubitem`/`deleteItem`/`reorderItem`. See [[2026-06-19-phase-6a-subitems-design]], [[2026-06-19-phase-6a-subitems]]. **Next: 6b — custom fields/statuses.**_
...
- **Phase:** Phase 6 (ClickUp depth) in progress — 6a Subitems shipped. **Next: 6b — custom fields/statuses** (spec + plan authored: `2026-06-19-phase-6b-*`), then C time-tracking · D relations+mirror · E docs. Full phase status above in §2 "Where we are".

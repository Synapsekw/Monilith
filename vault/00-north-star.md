...
- **Phase:** **Phase 10 (AI & Agents) — the assistant's write path is now genuinely usable.** Personal Agents Phase 2's board thread dock shipped and is **in production** ([[2026-08-04-1443-board-dock-and-ai-move-verb]]): a collapsible, resizable right dock hosting agent conversations, private by default and shareable to the board, with scheduled briefings landing as repliable threads and **zero requests until opened**. A permanent dock is chrome, reversing decision-27 ([[2026-08-04-decision-33-a-board-dock-reverses-ask-as-a-standalone-surface]]). Four AI write verbs exist (`create_item`, `move_item`, `set_item_fields`, `create_group`); as of [[2026-08-04-1907-promote-and-ai-write-followups]] they can all be **reached** (ids come from `query_items`, not only semantic search) and their results **render without a reload**. The cron chain remains live — E5, F13, F14, the weekly digest and agent briefings are all reachable in production. **E6 Stripe is the only open AI epic.**
- **Branch:** `main` @ `bfc238d3` == production (promoted 2026-08-04, **PR #83**, 31 commits — board dock + `propose_move_item`). Verified live: `www.monolith.works` 200, Vercel status `success`, main CI green. Squash divergence healed with `-s ours` (`46c8b117`), so `origin/main` is an ancestor of `origin/develop` ([[2026-06-30-gotcha-32-squash-merge-breaks-develop-main-ancestry]]) — do this **every** promotion or the next PR shows `CONFLICTING`. `develop` @ `e84e34ee` == `origin/develop`, ahead of `main` by the three AI-write follow-ups (item ids, org coupling, write visibility). **4329 tests**, ledger **131/131 in sync**, no worktrees, no open `task/*` branches.
...
- **A repair that matches zero rows reports success** ([[2026-08-04-gotcha-75-a-zero-row-repair-reports-success]]). `apply_migration` mis-stamping is now **5 for 5** — it is routine, budget it. But `reconcile-migration-version.sh`, the sanctioned fix, prints `WHERE name = '<slug>'` while the ledger stores the **full stamped filename**: the `UPDATE` touches nothing, exits 0, and looks repaired. Only `pnpm db:ledger-check` caught it. **Always run it after a reconcile.**
...
- **In flight:** none. All `task/*` merged and cleaned, `.claude/worktrees` empty.
- **Next:** **`/promote`** — `develop` leads `main` by the three AI-write follow-ups. Then the critical path is **Report Builder v2** (roll-ups + org templates: one migration dropping `reports.board_id NOT NULL`, then unwinding the single-board assumption across access checks, payload fetch, shaping, routing and ~1.4k lines of tests), or the **E6 Stripe track** (units B, C, E–H), which is **not** blocked on credentials — only end-to-end verification is. Cheap and overdue: fix `reconcile-migration-version.sh` and `finish-task.sh` (see Owed). Personal Agents Phase 2's remaining half — `@mentionable` agents in item threads, agent-initiated proposals, per-agent principals, PDF documents — is specced and unscheduled. **F17** (usage dashboard + AI weekly-digest narrative) survives only in the superseded July E6 plan and is tracked nowhere else.
- **Owed — tooling that lies.** (1) `reconcile-migration-version.sh`'s zero-row `UPDATE`, above. (2) `finish-task.sh` runs `rm -f vault/sessions/_draft-*.md` **before** its clean-tree check, so a *committed* draft breaks it for every session in the repo — one was committed in `023b4676` and removed; make that line skip tracked files. (3) `finish-task.sh` can exit 0 leaving an **orphaned vitest running against live DEV**; kill it and re-verify row counts, not just the directory. (4) Windows "Directory not empty" on worktree removal has **two** causes and the error text tells you which — *"used by another process"* is a real lock, *"Could not find a part of the path …"* is the `MAX_PATH` limit on deep `node_modules`, fixed only by `cmd /c rd /s /q "\\?\<path>"`. (5) Four stale remote branches (`task/perf-tier3`, `task/pwa-shell`, `task/rename-board-shared-tag`, `task/widget-preview-live`), 1 unmerged commit each since 2026-07-03 — inspect before deleting.
- **Owed — production.** `digest_secret`'s Vercel/Vault halves are still unverified against each other: a mismatch 401s and leaves **no trace**, because the route records a `blocked` row only when the variable is *absent*, never when it is wrong. Prod has **no `RESEND_API_KEY`**, so digest and briefing runs file in-app notifications and send no email — a run recorded as `ran` with no email is expected, not a fault. **The Ask Monolith write path is still unexercised on prod.** `net._http_response` is the first place to look when a cron is silently idle — it records the HTTP status of every `pg_net` call, which `cron.job_run_details` does not.

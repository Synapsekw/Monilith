...
- **Branch:** `develop == origin/develop` at `bee1149` (`main` behind — a promote is due). The **audit-fix wave (6 tasks)** all merged this session: `loading-skeletons`, `silent-empty-queries`, `onboarding-seam` (atomic+guarded org creation), `rls-confinement` (4-table cross-tenant write confinement, applied+verified on DEV), `status-pill-contrast` (shared StatusPill + dark-mode AA), `forgot-password` (self-serve recovery). **No live `task/*` worktrees** — all cleaned up. ([[2026-07-09-2207-audit-batch-a-fixes]])
- **In flight:** none building.
- **Next:** **promote the audit-fix wave `develop → main`** first, then **start the Keystone reskin tokens-first** in `src/app/globals.css` (user chose the **"Monolith Keystone"** direction from a 3-way live prototype pick; full token spec in [[dark-first-monday-reskin]]). Then two parallel tracks: **Phase 10 — AI & Agents E1** (starts with a scope-reconciliation pass — per-user BYO shipped vs org-scoped managed+metering+Ask Pulse plan; [[2026-07-05-1746-phase-10-ai-roadmap-scope]], [[2026-07-05-decision-26-ai-platform-dual-billing]]) and **PF — Polish & Fluidity** (20-task plan, `docs/superpowers/plans/2026-07-09-perf-polish-fluidity.md`; batch A first but **skip A4** route skeletons — shipped). A **Batch B** of product/hardening tasks (org switcher, auth rate limiting, notification prefs + due-date reminders, saved views, cached-reader membership assertions, sticky header + UI fragmentation) is scoped in the audit report §04.
- **Owed:** **Manual, blocking forgot-password in prod:** add `/auth/callback` to Supabase Auth **Redirect URLs** allowlist (DEV + PROD dashboards). **Promote due:** audit-fix wave `develop → main`. Perf tier-3 **Task A** (`unstable_instant`) — needs its own architecture spec ([[2026-07-04-gotcha-48-unstable-instant-blocked-by-shell-searchparams]]). **Landing redesign:** apply the chosen Keystone direction to `/landing` (converges with the app reskin) — remove `brand-lab` once it lands. **Phase 10 scope-reconciliation** (per-user BYO vs org-scoped managed plan).

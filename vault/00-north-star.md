...
- **Phase:** **Keystone secondary-surface polish in progress (Approach B — cluster worktrees, ship incrementally).** **Waves 0–1 promoted to prod** (PR#56, `main` at `e9ec8f2`): Foundation (`<MetaChip>` + `<ColorChip>`) + Board views (Calendar/Gantt/Kanban). **Waves 2–3 shipped to `develop`:** Wave 2 Dashboards (`6716ad7` — widget card 14px + `card-lift` + hairline, NumberWidget Kicker, ListWidget `<ColorChip>` cells) and Wave 3 Admin & entry (`bdd45c4` — Auth/Onboarding brand-washed entry cards + kicker eyebrows + glow CTA; Settings soft `<StatusPill>` + ADMIN kicker + input hairlines). Spec+plan `docs/superpowers/{specs,plans}/2026-07-10-keystone-secondary-surfaces-polish*.md`. Waves 4–6 remain (Planning → Personal/chrome → Core touch-ups). ([[2026-07-10-2058-keystone-wave3-admin-entry]], [[2026-07-10-2009-keystone-promote-and-wave2-dashboards]], [[dark-first-monday-reskin]])
- **Branch:** `develop == origin/develop` at `bdd45c4`; `main` at `e9ec8f2` (Waves 0–1 live; behind `develop` by Waves 2–3 + vault commits). **No live `task/*` worktrees.**
- **In flight:** none building.
- **Next:** **Keystone Wave 4 — Planning** (Goals · Portfolios · Workload) — recon → just-in-time plan → worktree → parallel subagents → gates → finish. Or `/promote` Waves 2–3 to prod first. Then Waves 5–6, and the two standing tracks — **Phase 10 — AI & Agents E1** (scope-reconciliation first; [[2026-07-05-1746-phase-10-ai-roadmap-scope]], [[2026-07-05-decision-26-ai-platform-dual-billing]]) and **PF — Polish & Fluidity** (`docs/superpowers/plans/2026-07-09-perf-polish-fluidity.md`; batch A first, **skip A4** — shipped). Scoped **Batch B** (org switcher, auth rate limiting, notification prefs + reminders, saved views, cached-reader membership assertions) remains in the audit report §04.
- **Owed:** **Waves 2–3 not yet promoted** — on `develop` only; promote as a batch or per-cluster. **Manual, blocking forgot-password in prod:** add `/auth/callback` to Supabase Auth **Redirect URLs** allowlist (PROD dashboard). **Keystone remaining:** Waves 4–6 (incl. sidebar wordmark mark, item-panel meta-chips/tab-counts, `@mention` highlighting, sidebar easing — folded into Wave 6). Deferred cleanup: DRY board `OptionPill` onto `<ColorChip>`. **Landing redesign:** apply Keystone to `/landing` — remove `brand-lab` once it lands (out of this pass). Perf tier-3 **Task A** (`unstable_instant`) needs its own spec ([[2026-07-04-gotcha-48-unstable-instant-blocked-by-shell-searchparams]]). **Phase 10 scope-reconciliation.**

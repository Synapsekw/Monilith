...
- **8 — Dashboards + templates + ⌘K polish** — <span style="color:#eab308">**[In progress — D1+D2 shipped]**</span>
  ...
  **Battery** (status-distribution bar) widgets grouping by a **Status** column; `shapeBuckets`
  ...
  (board-switch reset fixed). See [[2026-06-17-2119-dashboards-d2-chart-battery]]. **Next: D3**
  (List widget — bounded LIMIT row fetch + filter), then templates + ⌘K polish. Deferred:
  Dropdown/People grouping, per-widget config editing._
...
**Where we are:** Phases 0–4 done on `develop` **and now promoted to `main`** (PR #16 squash-merged 2026-06-17 — `main` carries Phases 0–4 + dark reskin + landing + sidebar; no Vercel project yet, so this is integration, not a live deploy). Phase 2 boards-core fully complete (**2c column management** add/rename/delete/resize shipped + pushed; 3b Calendar + Timeline/Gantt + dependencies shipped + user-verified). Dark reskin shipped. Board-view performance pass shipped. Collapsible sidebar shipped. **Public MONOLITH landing page shipped + pushed 2026-06-17.** **Phase 4 Collaboration complete (4a+4b+4c) + pushed 2026-06-17** — item detail panel (`?item=` drawer) + Updates + Activity Log (4a), @mentions + per-user notifications inbox (4b), and item-level **attachments** (4c — Supabase Storage + table/Storage RLS, Monday-style Files tab), all verified (310 tests, cloud RLS integration, Playwright e2e) with advisors clean. (`develop → main` promotion done 2026-06-17.) **Phase 8 started: Dashboards D1+D2 shipped + pushed 2026-06-17** — cross-board workspace dashboards (aggregation-RPC spine + drag-resize canvas + Number widget in D1; **Chart bar/pie + Battery** grouping by status in D2). Remaining near-term: user-verify D1+D2, Dashboards **D3** (List widget), light-mode reskin, then Phase 5 (Automations).

## 3. Now

- **Phase:** Phases 0–4 complete + promoted to `main`. **Phase 8 in progress — Dashboards D1+D2 shipped.** **Next:** user-verify D1+D2 in-app, then Dashboards **D3** (List widget) — closes the dashboards subsystem; then light-mode reskin / Phase 5.
- **Branch:** `develop` (synced with `origin/develop` at `376402e`). `main` at squash commit `30a9cf3`.
- **Latest (2026-06-17):** **Dashboards D2 — Chart + Battery widgets** (`818e4f1..376402e`, 8 commits, pushed). **Chart** (bar/pie, recharts v3.8.1) + **Battery** (CSS status-distribution bar) grouping a board's items by a **Status** column. No DB/RPC change — reused D1's `dashboard_aggregate`. Added chart/battery Zod schemas + pure `shapeBuckets`/`bucketsTotal` (None/Unknown/zero-count) + `getWidgetData` returns the group column's options as `columnMeta` (labels/colors resolve server-side) + generalized add-widget dialog (type/group/style). `useWidgetData` now returns `{ buckets, columnMeta }`; segment colors use each option's own hex. Gate green (typecheck/lint/**366 tests**/build); **live integration 5/5** (added grouped-by-status); e2e 2/2; review **SHIP-WITH-NITS** (fixed board-switch column reset). recharts v3 API verified against installed types (matched plan — see the major-version lesson [[2026-06-17-2048-gotcha-14-react-grid-layout-v2-api]]). See [[2026-06-17-2119-dashboards-d2-chart-battery]].
- **Prior (2026-06-17):** **Dashboards D1 — foundation + canvas + Number widget** (`708a7dc..7aa1fed`, 16 commits, pushed). Cross-board workspace dashboards: `dashboards`+`dashboard_widgets` tables (org-RLS) + `widget_kind` enum + `dashboard_aggregate` RPC spine (count/sum/avg, optional status grouping, empty-cell LEFT JOIN) + Server Actions + queries + TanStack hooks (per-widget data keyed by config hash) + `react-grid-layout` **v2** drag-resize canvas (debounced layout persist, **0 refetch on drag**) + Number/KPI widget + add-widget dialog + sidebar Dashboards section. 15-task subagent-driven; gate green; **live cross-org RLS + aggregate 4/4**; Playwright e2e green; review **SHIP-WITH-NITS**. Plan deviation: rgl is v2.2.3 (rewritten API) → [[2026-06-17-2048-gotcha-14-react-grid-layout-v2-api]]. See [[2026-06-17-2048-dashboards-d1-foundation]].

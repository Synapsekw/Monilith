...
- **Phase:** Phases 0–4 complete + promoted to `main`. **Phase 8 COMPLETE**. **RS workstream COMPLETE** (dark + light). **Phase 5 in progress — 5a + 5b-1 + 5b-2 + 5c-1 (run-history) all shipped; 5c-1 built this session, not yet user-verified live.** **Next:** **5c-2** (external/webhook actions via `pg_net`), or user-verify 5c-1 live.
- **Branch:** `develop` (**ahead of `origin/develop` by 12, 0 behind** — 5c-1's 8 commits + a parallel session's name-column work; nothing pushed this session). `main` at squash commit `30a9cf3` — **do not promote yet** (WebGL landing dep needs a manual cross-browser check; develop also carries in-flight name-column work).

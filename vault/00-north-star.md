...
- **Branch:** `develop == origin/develop` at `6274887` (squash-heal merge). Last production promotion is **#36** (2026-06-25, `564394c` on `main` — relocate dashboards generate-with-AI into the Plus dropdown), on top of **#35** (`674c570` — calendar item-panel open, calendar canvas-fill, Phase 9.4 skeletons, Phase 9.3 shell caching). `main ⊆ develop` (healed, gotcha-32); even with `main` in content. ([[2026-06-25-2052-batch-a-build-and-promote]])
- **In flight:** nothing open; main-checkout tree clean. Last shipped: Batch A (#35) + dashboards AI-button relocation (#36, `bb5923e` — Generate-with-AI moved from a standalone button into the Plus dropdown; same lazy-loaded wizard, no backend change). Two other-session worktrees still live (`streaming-shell-9-2` stale, `board-spreadsheet-io`).

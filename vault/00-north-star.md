...
- **Branch:** `develop == origin/develop` (`7e104db`, pushed 2026-06-21 — adds attachment storage cleanup on delete + in-app invite acceptance on top of 6f PDF-preview + board-level sharing; typecheck/lint/build green, CI green). `main` at `30a9cf3` (Phases 0–4) — **do not promote yet** (WebGL landing dep needs a manual cross-browser check). _(Parallel sessions share this checkout — a concurrent branding edit to `page.tsx` is uncommitted in the tree; leave it untouched; commit only your own paths.)_

...
- **Phase:** **`develop` synced + a DEV-only migration brought under version control** ([[2026-07-25-0821-develop-sync-acl-migration-backfill]]). Routine post-pull ledger hygiene caught `20260724134101_mcp_oauth_vault_cleanup_acl` applied to DEV with **no committed file** — the ACL lockdown on `oauth_tokens_vault_cleanup()` (SECURITY DEFINER, deletes from `vault.secrets`), which the main MCP-OAuth migration missed. Backfilled at the ledger version and written up as [[2026-07-25-gotcha-57-dev-applied-migration-with-no-committed-file]]: no existing gate catches this shape (gotcha-55's reconcile needs the file to exist, `db push` is file-driven, and a grant-only migration leaves `db:types` identical). Also confirmed no session note is owed — the E5 note was in an unpushed local commit, not stranded on another machine, and the north-star's own "never committed" prose was corrected. The feature front is unchanged: **MCP server** ([[2026-07-24-1950-mcp-server-oauth]]) — hosted remote MCP server + OAuth 2.1 authorization server letting Claude Desktop/claude.ai connect as a real Pulse user over a genuinely RLS-scoped per-request session (6 read/write tools, no delete) — and **E5** both sit on `develop`, ahead of prod.
- **Branch:** `develop` @ `db2be59`+wrapup == `origin/develop` (MCP server + E5 agentic/semantic + the ACL migration ahead of prod — everything else promoted via #69). **`main` @ `8b4d648`**. All `task/*` worktrees merged + cleaned.
- **In flight:** none building.
- **Next:** **promote `develop → main`** (deferred to the owner) — it now carries MCP server + E5 **+ the ACL migration**; E5 needs its env vars set + an embeddings backfill first (see Owed). A real end-to-end MCP connection test (Claude Desktop/claude.ai against a deployed `develop`) still hasn't been run — do that before calling MCP done-done. Or a fresh roadmap build — **Report Builder v2** (charts + wide-board table) or **E6** (Stripe billing; F16 blocked on your creds).
- **Owed:** **MCP server follow-ups:** `/login` doesn't honor `?next=` so the OAuth connect flow can't resume post-login for a not-yet-signed-in user (pre-existing app-wide gap, first flow it actually bites); revoke-button errors aren't surfaced in Settings UI; `/api/oauth/register` has no rate limit. **Carry `20260724134101_mcp_oauth_vault_cleanup_acl` to prod:** that ACL lockdown was applied to DEV but had no committed file until 2026-07-25 — it must ride the next `/sync-prod`, or prod ships the definer trigger function publicly executable ([[2026-07-25-gotcha-57-dev-applied-migration-with-no-committed-file]]). Follow-ups from it: nothing diffs `list_migrations` against `supabase/migrations/` (teach `finish-task.sh` or a `/sync-prod` pre-check), and the other definer functions added 07-20/07-24 want the same ACL audit. **E5 pre-promotion:** set E5 env vars, then backfill embeddings (`POST /api/ai/embed?mode=backfill`) so semantic surfaces actually return results once promoted — full detail in [[2026-07-21-1123-e5-agentic-semantic-full-build]], which landed in this vault on 2026-07-25. **Report Builder v2** = charts + wide-board table continuation. **Wordmark mark** in prod — one-line revert in `brand.tsx` if busy. **Rotate the prod DB password** (still owed from an earlier `/sync-prod`).

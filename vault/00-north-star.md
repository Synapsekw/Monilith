...
- **TOUCH — iPad optimization** — <span style="color:#eab308">**[In progress — Batch 1 foundation done]**</span> — iPad-first, full authoring parity, touch-ergonomics (no layout reflow). Batch 1 shared primitives shipped: `useCoarsePointer`, touch-aware dnd sensors (long-press lift), `<DragHandle>`, `<RevealOnHover>`, 44px tap targets, touch-aware tooltip; Kanban reference integration. Batch 2 = 8 per-surface plans against the merged API (Table/Kanban/Gantt+zoom/Nav/Calendar/Dashboard/Item Panel/⌘K). Spec `docs/superpowers/specs/2026-06-26-ipad-touch-optimization-design.md`. ([[2026-06-28-1822-ipad-touch-foundation]])

## 3. Now

- **Phase:** Phase 6 (ClickUp depth) complete except deferred 6e Docs · Phase 7 (Asana polish) in progress — 7a + 7b + 7c + Workload analytics v3 + **time allocation** shipped · **Phase 9 (Hardening) DONE** · **TOUCH (iPad) Batch 1 foundation DONE** — Batch 2 per-surface work pending.
- **Branch:** `develop == origin/develop` at `eb16d70`. Unpromoted on `develop` since **#36**: **item creation tracking** (`c11d4e8`), **9.5a interaction responsiveness** (`3a6880f`), board spreadsheet import/export (`1d7d578`) + **review follow-ups** (`384ccf1`), integration-test flake fix (`56180fb`), **sidebar share-icon polish** (`274cdf2`), **9.3b widget-aggregation cache** (`b546595`), **9.6 Web-Vitals gate** (`a264fee`), **iPad touch foundation (Batch 1)** (`eb16d70` — 8 commits of shared touch primitives + Kanban sensor integration), plus docs/tooling: `/sync-prod` spec+plan, **`/roast` decision-council command**, Docker refs removed from the test-DB plan (`2850a6f`). Last production promotion is **#36** (2026-06-25, `564394c` on `main`). `main ⊆ develop`.
- **In flight:** nothing open; main-checkout tree clean (only unrelated `.obsidian/*` edits pending, left untouched).
- **Next:** run `/promote` to ship the unpromoted `develop` bundle (item creation tracking + 9.5a + spreadsheet io + review follow-ups + flake fix + sidebar share-icons + 9.3b + 9.6 + **iPad touch foundation** + roast docs) → then either **TOUCH Batch 2** (first per-surface plan — suggest Kanban full pass or Item Panel) or Phase 7 remaining polish / Phase 10. Optional carryover: optimistic board mutations — **recommended skip**. Note: Lighthouse budget numbers confirm on first CI run.

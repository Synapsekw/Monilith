...
- **Phase:** **MVP-F DONE — 9/9 shipped to prod** · **Phase 7 confirmed fully built** · **Import Wizard v2 merged** · **Four carryover plans built + merged to develop** (rename-board-shared-tag, pwa-shell, widget-preview-live, perf-tier3 5/6). Standing product call next: define Phase 10, revive deferred 6e Docs, or **declare v1 feature-complete** (my read: v1-complete). ([[2026-07-04-1107-parallel-build-four-scoped-plans]])
- **Branch:** `develop == origin/develop` at `e2ffaa3` (four-plan build wave) · `main` at `f0f71f5`, prod deploy confirmed. Feedback rows: F1/F3/F6 resolved, F2/F5 in_progress (flip after prod confirms Batch B).
- **In flight:** nothing building — the four scoped plans all shipped to develop. No open `task/*` worktrees.
- **Next:** make the **v1-feature-complete call** (or define Phase 10). Then either (a) spec the shell-off-`useSearchParams` refactor if instant-nav is still wanted ([[2026-07-04-gotcha-48-unstable-instant-blocked-by-shell-searchparams]]), or (b) work the two owed migrations below. Prod promotion of the four-plan wave (`develop → main`) is available once you're ready.
- **Owed:** (0) **re-apply import-wizard-v2's dev migration** — paste `supabase/migrations/20260703110000_import_rows_into_board.sql` into the DEV SQL editor (adds the `can_edit_board` guard; idempotent, no `db:types` churn). prod Batch B migrations. Migration-ledger repair on BOTH projects at next `db push`/`/sync-prod` (recent applies were SQL-editor, incl. `…703110000`). Health-summary requester questions in its spec. `.mcp.json` uncommitted; untracked `scripts/sync-prod/push-schema.sh` (foreign). Perf tier-3 **Task A** (`unstable_instant`) deferred — needs its own spec.

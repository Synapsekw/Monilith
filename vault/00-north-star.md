...
- **Phase:** Phases 0–4 complete + promoted to `main`. **Phase 8 COMPLETE**. **RS workstream COMPLETE** (dark + light). **Phase 5 in progress — 5a + 5b-1 + 5b-2 + 5c-1 (run-history) all shipped.** Side: **Name-column auto-fit + manual resize** + **Add-group (board groups creatable from the Table)** both shipped + user-verified 2026-06-19. **Next:** **5c-2** (external/webhook actions via `pg_net`), or user-verify 5c-1 live.

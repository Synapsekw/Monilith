...
- **Branch:** `develop` @ `c72bc02` == `origin/develop`, **4 migrations ahead of prod** (`20260725102610` ACL lockdown + 3 account-deletion). **`main` @ `186782d`** (promotion #72 shipped MCP server + E5 + settings redesign). All `task/*` worktrees merged + cleaned; ledger 115 files = 115 DEV rows.
- **In flight:** none building.
- **Next:** **`/sync-prod` FIRST, then promote `develop → main`** — the order is load-bearing: promoting first ships a Delete Account button with no reassignment RPC behind it. Then provision prod Vault + the two Vercel env vars, run the embeddings backfill, and finally the real end-to-end MCP connection test (now genuinely possible for the first time). After that: **Report Builder v2** (charts + wide-board table), **Ask Pulse Phase 2** (write actions in `/ask` — the write engine and confirm card already exist, S–M), or **E6** (Stripe; F16 blocked on your creds).
- **Owed:** **PROD IS STILL EXPOSED** — all 8 anon-executable definer functions remain live in prod until `/sync-prod` runs; promotion does not apply migrations. **Prod Vault is empty** (no `app_url`, `ai_pgnet_hmac_secret`, `digest_secret`) and `/sync-prod` pushes schema, not Vault secrets — an unguarded structural gap. Two shipped features are silently inert because of it: E5 semantic search (`item_embeddings` = 0 rows vs 380 live items) and **the health digest, which has never fired in prod since 2026-07-03 (`digest_runs` = 0 rows ever)**. Crons report `succeeded` while skipping, so nothing alerts. Provisioning `digest_secret` starts emailing real users with three weeks of backlog — needs an explicit decision. **E5 env blocked:** the two `vercel env add` commands were denied by the permission classifier; `.env.local` is corrected locally (`OPENAI_EMBEDDING_API_KEY` is the only name the code reads). **Finding F1:** MCP assigns people without ever notifying them — real, user-visible, fix is a one-line swap once `upsertCellCore` is hoisted ([[2026-07-25-gotcha-60-server-action-side-effects-invisible-to-mcp]]). **Integration suites still skip** (no `.env.test`) — a skip is not a pass. **E6** Stripe is the only genuinely open Phase-10 epic; E2/E3/E4 shipped in #62.

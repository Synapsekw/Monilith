...
- **Phase:** **`/updates` had silently stopped tracking what we ship — now backfilled** ([[2026-07-31-1559-changelog-backfill-updates-page]]). Ten days and two promotions (#72, #73) had gone by without a single `Changelog:` trailer: **19 user-facing changes** missing, including the Claude connector, the Settings redesign and self-serve account deletion. Backfilled as six announcement commits, one per ship date with the **author date pinned** so `groupByDate` renders July 17-27 as six accurate headings rather than one lump.
- **The CI gate could not have caught this, and the gap was self-evidencing.** `git diff --exit-code generated.ts` proves the file **matches** history — it is a consistency check, never a coverage one, so a feature that ships with no trailer produces no diff and the gate is green precisely when the changelog is wrong. Two entries published on 07-27 described refinements to a connector whose own announcement had never been written: the page improved a feature it had never shipped, and nobody noticed ([[2026-07-31-gotcha-64-changelog-drift-check-cannot-see-a-missing-trailer]]). The check now belongs on the **promotion PR** (`git log main..develop` vs its trailers) — backfilling is lossless, so deferring it is acceptable.
...
- **Branch:** `main` @ `1121796` == production. `develop` @ `e69e6f0c` == `origin/develop` — 7 commits ahead of `main`, all changelog trailers plus one `generated.ts` regen; **no source behaviour changed**, so promotion is documentation-only apart from the ordering constraint below. Ledger **118 files = 118 DEV rows = 118 PROD rows**. All `task/*` worktrees merged + cleaned.
- **In flight:** none.
- **Next:** **promote `develop → main` to publish the 19 entries — but drain the embedding queue first** (see Owed; two of the entries announce semantic search, which currently returns nothing on prod). Then **Report Builder v2 roll-ups + org templates** as the critical path — one migration dropping `reports.board_id NOT NULL`, after which the single-board assumption must be unwound across access checks, payload fetch, shaping, routing and ~1.4k lines of tests. **E6** Stripe remains blocked on your creds.
- **Owed — three items, all needing _you_ rather than an agent.** **E5 semantic search is still inert** (`item_embeddings` = 0 vs 439 live prod items) — and this is now a **promotion blocker, not just a nice-to-have**: the backfilled changelog announces "Find similar items" and "Ask AI searches by meaning" the moment `main` advances. `OPENAI_EMBEDDING_API_KEY` **was** provisioned on Vercel 2026-07-25; all that remains is enqueueing existing items into `item_embed_queue`, after which the `embed-sweep` cron drains 50 every 2 min and signs its own call — no secret handling needed. **`supabase-prod` MCP is read-only (`25006`)**, so that enqueue must be pasted into the prod SQL editor. **`digest_secret` is still absent from prod Vault**, but the schema half is live, so provisioning is finally safe: ping `_health_digest_ping()`, confirm a `blocked` row naming the secret appears (the alarm that was missing for three weeks), then add it — noting prod has **no `RESEND_API_KEY`**, so the first runs file in-app notifications and send **no email** regardless. **The Ask Monolith write path is still unexercised on prod.**

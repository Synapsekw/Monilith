...
- **Branch:** `develop == origin/develop` at `15d04f5` (vault commits may sit on top locally). Last production promotion is **#30** (2026-06-23, `f10cd8c` on `main` — Workload v2 filtering+actuals, time-tracking date Calendar, people-cell names). `develop` is **ahead of `main`** since #30 (Phase 9.2 streaming shell, Workload analytics v3, Kanban card names, auth `getClaims` proxy, sidebar board drag-reorder, Feedback reporting, Dashboards v2 polish, percent (progress) column **+ value-based color + collapsed-group rollup**, **Timeline two-column spans + colorize-by-status**, time allocation, column-add error surface, Sum/Average measure fix, `/updates` changelog backfill, shared `(app)` shell layout, reorder no-shell-reload, **AI dashboard generation (Opus 4.8)**, Kanban card polish (Direction A) — none promoted yet).
- **In flight:** nothing open. Last edit: **integration-test timeout flake mitigation** ([[2026-06-24-2311-integration-test-timeout-flake]]) — diagnosed a flaky `pnpm test` run (`11 files / 9 tests` failed) as load-induced timeout flake, not a regression (both projects pass 100% in isolation + combined); added `testTimeout 30s / hookTimeout 60s / retry: 1` on the integration project (`vitest.config.ts`, `1e93f03`). Prior edit: **Timeline two-column spans + colorize** ([[2026-06-24-2232-timeline-spans-colorize]]). Other sessions' worktrees are live (`calendar-redesign`, `streaming-shell-9-2`); their unrelated uncommitted edits (`DashboardsNav`, deleted `GenerateWithAiButton`) sit in the main checkout, untouched.

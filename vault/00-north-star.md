...
- **Branch:** `develop == origin/develop` at `c549f82` (vault commits may sit on top locally). Last production promotion is **#30** (2026-06-23, `f10cd8c` on `main` — Workload v2 filtering+actuals, time-tracking date Calendar, people-cell names). `develop` is **ahead of `main`** since #30 (Phase 9.2 streaming shell, Workload analytics v3, Kanban card names, auth `getClaims` proxy, sidebar board drag-reorder, Feedback reporting, Dashboards v2 polish, percent (progress) column **+ value-based color + collapsed-group rollup**, **Timeline two-column spans + colorize-by-status**, **Calendar timeline redesign (multi-day spanning bars + Month/Week/Agenda)**, time allocation, column-add error surface, Sum/Average measure fix, `/updates` changelog backfill, shared `(app)` shell layout, reorder no-shell-reload, **AI dashboard generation (Opus 4.8)**, Kanban card polish (Direction A) — none promoted yet).
- **In flight:** nothing open. Last edit: **Calendar view timeline redesign** ([[2026-06-24-2327-calendar-timeline-redesign]]) — multi-day spanning bars in greedy-packed lanes + **Month/Week/Agenda** toggle (client-state, 0-refetch), Month capped 3 lanes + "+N more" day popover, neutral single-day bars vs status-filled spans, Unscheduled drawer removed; built subagent-driven (8 tasks + whole-branch review caught a cross-week dnd-id collision), merged `c549f82`. Prior edit: **integration-test timeout flake mitigation** ([[2026-06-24-2311-integration-test-timeout-flake]]). Other session's worktree still live (`streaming-shell-9-2`); the unrelated uncommitted edits (`DashboardsNav`, deleted `GenerateWithAiButton`) sit in the main checkout, untouched.
- **Next:** run `/promote` to ship the `develop` bundle (now includes AI generation) — **first add `ANTHROPIC_API_KEY` to Vercel (Production + Preview)** or the deployed AI button errors → optional: make rename/delete/create/duplicate optimistic too (shared client boards store) → 9.3 cache + 9.4 skeletons → 9.6 Web-Vitals gate.
- **Owed:** production promotion of the `develop` bundle (run `/promote`) — gated on adding `ANTHROPIC_API_KEY` to Vercel first. Re-run the integration suite for the recent unit-gated merges once Supabase is stable. **Integration-test gate is flaky AND writes to production** — the live-DB `*.integration.test.ts` suites run against the **remote/prod** Supabase (Vitest loads `.env.local`), so they fail nondeterministically _and_ leak fixtures (`*@example.com` users, `Org a`/`rls-*`/`eng5b1-*` orgs) into real tenant data when a test throws before teardown. Timeout-under-load is now partly mitigated (`testTimeout/hookTimeout/retry` on the integration project, `1e93f03`), but the real fix is unchanged: local Docker Supabase (`supabase start` → `127.0.0.1:54321`) via `.env.test` — isolates + makes the gate reliable ([[2026-06-23-gotcha-43-shared-db-integration-test-flake]], [[2026-06-24-0751-test-fixture-prod-db-cleanup]], [[2026-06-24-2311-integration-test-timeout-flake]]).

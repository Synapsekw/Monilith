...
- **In flight:** nothing open. **`/sync-prod` completed this session** (2026-06-29) — dev → prod full replace, parity verified (orgs 209, boards 168, items 360, users 230, storage 9 — all match); prod backed up first (`prod-backup-20260629-140950.sql`). ⚠️ Prod now carries DEV's ~220 test fixtures (`@example.com`/`rls-*`/`eng5b1-*`) — durable fix is the planned isolated test DB (`.env.test`) so fixtures stop reaching the dev→prod source.

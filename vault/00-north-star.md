...
- **Branch:** `develop == origin/develop` (`ccb5162`, pushed 2026-06-21 — adds the worktree-per-session workflow + pinned commit identity on top of attachment cleanup + in-app invite acceptance; CI green). `main` at `30a9cf3` (Phases 0–4) — **do not promote yet** (WebGL landing dep needs a manual cross-browser check). _(Parallel sessions share this checkout — a concurrent branding edit to `page.tsx` is uncommitted in the tree; leave it untouched; commit only your own paths.)_

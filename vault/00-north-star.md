...
- **Phase:** **The promotion landed and the cron chain is live.** `develop → main` merged as `8755f9de` (2026-08-02 03:38 UTC), publishing the proxy fix, the new landing and Personal Agents Phase 1. Verified empirically, not assumed: `_health_digest_ping()` → `net._http_response` = **200**, where it read **405** an hour earlier. The five integrations that had never worked in production — E5 semantic search, F13 automation steps, F14 Autopilot, the weekly digest, agent briefings — are reachable for the first time ([[2026-08-01-gotcha-69-a-cookie-gate-turns-a-cron-post-into-a-silent-405]], [[2026-08-01-1146-debt-audit-and-paydown]]).
...
- **Branch:** `main` @ `8755f9de` == production (promoted 2026-08-02, PR #79). `develop` @ `fddd74ca` == `origin/develop`, ahead of `main` by **agent run history + the Phase-1 hardening migration** (`38ccf828`). Ledger **verified in sync** — the MCP version drift recurred exactly as documented (`…034441` stamped for file `…034242`) and `reconcile-migration-version.sh` fixed it; this is now a routine step, not a surprise. All `task/*` worktrees merged + cleaned; `ai-cogs` belongs to a concurrent session.
- **In flight:** none.
- **Next:** ~~Confirm E5 drained~~ — **DONE, verified 2026-08-02 04:11 UTC on the LIVE database (`hjqca…`): queue 0, 384 embeddings, all item-backed, seven clean sweeps of 50.** Semantic search works in production for the first time. ~~Decide signup-vs-waitlist~~ — **CLOSED 2026-08-02: owner reviewed the promoted landing and accepts it as-is.** The mixed model (nav "Get started → `/signup`", footer "Invitation only", inert waitlist input) is a deliberate choice, not an outstanding defect — do not re-raise it as one. Next is **Report Builder v2 roll-ups + org templates** as the critical path — one migration dropping `reports.board_id NOT NULL`, after which the single-board assumption must be unwound across access checks, payload fetch, shaping, routing and ~1.4k lines of tests. **E6** Stripe remains blocked on your creds. Personal Agents **Phase 2** (thread dock, `@mentionable` agents, propose-writes, per-agent identities) is specced and unscheduled.
- **Owed.** **E5 is DONE and verified** — but the earlier revision of this bullet (and the work behind it) targeted the **wrong database**. Per [[2026-08-02-decision-32-production-runs-the-dev-database]] the live site runs **`hjqca…` (DEV)**; `jzsyq…` serves no traffic, so its Vault secrets and its 380-row queue were irrelevant. The live chain needed **three** faults fixed, each masking the next: the cookie gate (405), an absent `app_url`, and a Vault/Vercel **HMAC mismatch** (401) — all three logged `succeeded` in `cron.job_run_details`; only `net._http_response` distinguished them ([[2026-08-01-gotcha-69-a-cookie-gate-turns-a-cron-post-into-a-silent-405]]). **Never infer the environment from a variable name — `PROD_SUPABASE_DB_URL` points at the idle project.** **Still open:** the four remaining `pg_net` crons on `jzsyq…` (`automation-ai-reconcile`, `autopilot-sweep`, `health-digest-ping`, `personal-agent-sweep`) all post to `www.monolith.works` and therefore mutate **live** data from the idle mirror's stale view; `embed-sweep` there is already unscheduled and the rest should follow until cutover. **`digest_secret` is now PROVISIONED** (2026-08-01) — present in prod Vault, 64 chars, and its `DIGEST_SECRET` twin was set on Vercel; the DB half is fingerprint-verified. The alarm an earlier revision called missing had in fact already fired: `digest_runs` holds a `blocked` row dated 2026-07-28 naming the secret, which is what made provisioning empirically safe. **Unverified half:** that the Vercel value matches the Vault one — a mismatch 401s and leaves **no trace**, because the route records a `blocked` row only when the variable is *absent*, never when it is wrong. First real exercise is the **next 07:00 UTC run**, which is now the first one that can actually reach its handler. Prod has **no `RESEND_API_KEY`**, so the first runs file in-app notifications and send **no email** regardless — the same is true of agent briefings, so a run recorded as `ran` with no email in your inbox is expected, not a fault. **The Ask Monolith write path is still unexercised on prod.** **Direct prod psql works** via `PROD_SUPABASE_DB_URL` in `.env.prod.local`, so the "must paste into the SQL editor" constraint is obsolete for reads — though the auto-mode classifier blocks agent *writes* to prod, so those still need your hand.

...
The owner likes this look and wants Monolith to adopt it. Monolith already commits to the same _philosophy_
...
1. **Dark-first.** The dark near-black theme becomes Monolith's primary/reference look; light mode stays
...
3. **Reuse the prototype as a UI/feature donor, never an architecture donor.** Keep Monolith's spine
...
4. **Translate, don't copy, tokens.** The prototype is Tailwind v3 with a `tailwind.config.js` hex
   palette. Monolith is **Tailwind v4** (no config file — `@theme` in `globals.css`, OKLch vars). Port the
   palette by translating hex → OKLch `@theme` tokens; never hardcode hex. (The "we're not using
   Tailwind" worry was a false alarm: Monolith _is_ on Tailwind v4, which simply has no `tailwind.config.js`.)
...
- `lib/exporters.ts` (JSON/CSV export + file import) → wire to Monolith cache/actions. _(satisfies PRD X-6)_
...
- `views/CalendarView.tsx`, `views/TimelineView.tsx`, `views/DashboardView.tsx` (Recharts) — Monolith has
...
- `timeline`, `progress`, `rating`, `link`, `files`, `formula`, `longtext`, `checkbox`, `priority`.
  Monolith currently supports only `text | status | people | date | numbers | dropdown`.

**Do NOT port:** the Zustand store (`store.ts`), localStorage persistence, undo/redo-via-module-arrays,
`Sidebar.tsx` / `TopBar.tsx` (Monolith has its own nav + `ViewSwitcher`), and the bespoke `ui.tsx`
Popover/Modal (Monolith uses Radix/shadcn equivalents).

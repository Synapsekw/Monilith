...
// Phase-9 performance budget (spec §"Success budget"). A budget edit that
// silently drops or weakens an assertion fails here.
type Assertion = [
  level: string,
...
describe(".lighthouserc.json", () => {
  ...
  it("enforces TTFB (server-response-time) < 200ms", () => {
    expect(assertions["server-response-time"]?.[0]).toBe("error");
  ...
  it("enforces a first-load JS (script size) budget", () => {
    expect(assertions["resource-summary:script:size"]?.[0]).toBe("error");
    expect(

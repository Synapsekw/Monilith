...
describe("Sub-item rows with an active intelligence chip", () => {
  it("marks the matching sub-item intel-match and dims its parent (kept for context)", () => {
    ...
    expect(
      within(parentRow as HTMLElement).queryByTestId("intel-rule"),
    ).toBeNull();
    ...
    expect(subRow).toHaveAttribute("data-intel-rule", "cell");
    expect(
      within(subRow as HTMLElement).getByTestId("intel-rule"),
    ).toBeInTheDocument();

# `/roast` Adversarial Council Command — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `/roast` slash command that stress-tests an idea or decision via a parallel adversarial council of persona sub-agents plus a separate Judge.

**Architecture:** A single prose orchestration command at `.claude/commands/roast.md`, modeled on the existing `whats-next.md`. It detects mode (idea/technical), runs a ≤3-question intake gate, assembles a brief, dispatches 5 persona agents in one parallel `Agent` batch, dispatches a Judge agent over their reports, and renders a chat-only verdict. No app source, no worktree, no persistence.

**Tech Stack:** Claude Code slash command (markdown), `Agent` tool (general-purpose) for fan-out, `AskUserQuestion` for intake, WebSearch/WebFetch inside the Deep Researcher agent.

**Spec:** `docs/superpowers/specs/2026-06-26-roast-council-design.md`

**Note on TDD:** This deliverable is a prose orchestration command, not compiled code — there is no unit test to write first. Verification is done via live dry-runs (Tasks 2–4), which are the "tests" for this artifact, per the spec's Testing section and AGENTS.md ("verification scales to what is built").

---

## File Structure

- Create: `.claude/commands/roast.md` — the entire deliverable. One file, one responsibility (orchestrate the council). Registers as `/roast`.

No other files are created or modified.

---

### Task 1: Author the `/roast` command file

**Files:**

- Create: `.claude/commands/roast.md`

- [ ] **Step 1: Write the command file**

Write `.claude/commands/roast.md` with exactly this content:
...
Run: `test -f .claude/commands/roast.md && wc -l .claude/commands/roast.md`
Expected: the file exists and reports a non-trivial line count (~100+ lines).

- [ ] **Step 3: Commit**

```bash
git add .claude/commands/roast.md
git -c user.name="Danijel Jovanovic" -c user.email="info@synapse-solutions.ai" \
  commit -m "feat(roast): adversarial decision council slash command
...
- [ ] **Step 1: Confirm convention match**

Read `.claude/commands/roast.md` and `.claude/commands/whats-next.md` side by side. Confirm
`roast.md`:

- opens with a `# /roast — …` H1 header line (tagline form),
- uses numbered `### N.` step sections,
- instructs a TodoWrite item per step.

Expected: all three hold. Fix `roast.md` inline if any are missing.

- [ ] **Step 2: Confirm it registers**

The command registers from the file's presence in `.claude/commands/`. Confirm the filename is
exactly `roast.md` (so it registers as `/roast`).

Expected: filename correct.

---

### Task 3: Live dry-run — idea mode

**Files:** none (behavioral verification)

- [ ] **Step 1: Run `/roast` on a sample idea**

Invoke: `/roast a $9/mo tool that turns a YouTube transcript into a week of LinkedIn posts`

- [ ] **Step 2: Confirm behavior**

Verify, in order:

1. Mode is detected as **idea** (printed in one line).
2. Intake asks **≤3** questions covering buyer / edge / constraints (skipping any already answered).
3. After intake, **5 persona agents dispatch in a single parallel batch** (Contrarian, Expansionist,
   First-Principles, Deep Researcher, Buyer).
4. The Deep Researcher makes **real web calls** (its report cites competitors/pricing/sources).
5. A **separate Judge** runs after the personas.
6. The verdict renders with: VERDICT + confidence, one-line call, WHY, a **SCORES markdown table**
   with all 5 personas, and a **48-hour CHEAPEST TEST**.

Expected: all six hold. If any fails, fix `roast.md` and re-run.

---

### Task 4: Live dry-run — technical mode

**Files:** none (behavioral verification)

- [ ] **Step 1: Run `/roast` on a sample technical decision**

Invoke: `/roast should we replace our Zustand board store with React Context for the board view`

- [ ] **Step 2: Confirm behavior**

Verify:

1. Mode flips to **technical**.
2. Intake questions change to goal / constraints / alternatives.
3. The roster swaps **Buyer → Maintainer** (Contrarian, Expansionist, First-Principles, Deep
   Researcher, Maintainer dispatch in one batch).
4. The verdict's CHEAPEST TEST is a **validation spike**, not a 48-hour market test.

Expected: all four hold. If any fails, fix `roast.md` and re-run.
...
Expected: all pass — no app source changed, so this only proves the addition broke nothing.

- [ ] **Step 2: Confirm the branch is clean**

Run: `git status --short` and `git log --oneline -2`
Expected: `roast.md` committed; no stray staged changes outside the command file and spec.

- [ ] **Step 3: Hand the user a "How to test this" walkthrough**

Provide a numbered manual-test guide: run `/roast <an idea>`, answer the intake, watch the 5
parallel agents + Judge, read the verdict; then run `/roast <a technical decision>` and confirm the
Maintainer swap. (This is tooling, not user-facing app behavior, so the walkthrough targets the
...
- **Spec coverage:** mode detection (Task 1 §1), intake gate (§2), brief (§3), 5-persona parallel
  fan-out (§4), separate Judge (§5), chat-only verdict + schema (§6), idea+technical rosters
  (Task 1 roster + Tasks 3/4), live-web Deep Researcher (Task 3 step 2.4), structural check
  (Task 2), both dry-runs (Tasks 3–4), gates (Task 5). All spec sections map to a task.
- **Placeholder scan:** none — the full command file content is inline in Task 1.
- **Type/name consistency:** persona names (Contrarian, Expansionist, First-Principles, Deep
  Researcher, Buyer, Maintainer), the output schema, and the verdict schema are identical across
  the spec, the command file, and the verification tasks.

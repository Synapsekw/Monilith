...
_"aggregation — sum/avg/min/max/count in the column-summary footer"_). Scoped to the
...
- **F1** — Subitems excluded from the board total (top-level rows only). _Default: exclude._
- **F2** — Numbers requires an explicit pick (no auto-default to Sum). _Default: opt-in._

...
- **No provider-adapter interface change.** v1 requires **Anthropic** for the three features (mirrors
  Ask Monolith's `ProviderNotCapableError` guard). Generalizing structured output onto the adapter is a
